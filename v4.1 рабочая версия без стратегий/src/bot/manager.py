...
class BotManager:
    ...
    async def _generate_signal(self, symbol: str, market_data: Dict[str, Any]) -> Optional[Signal]:
        """
        Исправленная генерация сигналов
        """
        try:
            ...
            if auto_strategy_selector:
                try:
                    ...
                    logger.info(f"✅ Выбрана стратегия '{best_strategy_name}' для {symbol} с уверенностью {strategy_confidence:.1%}")
                    
                    # Обновляем стратегию в БД если изменилась
                    if hasattr(pair_config, 'strategy') and pair_config.strategy != best_strategy_name:
                        await self._update_pair_strategy(symbol, best_strategy_name)
                        pair_config.strategy = best_strategy_name
                        
                except Exception as selector_error:
                    logger.error(f"❌ Ошибка селектора стратегий: {selector_error}")
                    # Используем текущую стратегию пары или дефолтную
                    best_strategy_name = getattr(pair_config, 'strategy', 'multi_indicator')
                    strategy_confidence = 0.7
            else:
                # Используем стратегию из конфигурации пары или дефолтную
                best_strategy_name = getattr(pair_config, 'strategy', 'multi_indicator')
                strategy_confidence = 0.8
                
            # === ШАГ 3: СОЗДАЕМ ЭКЗЕМПЛЯР СТРАТЕГИИ ===
            strategy = self._create_strategy_instance(best_strategy_name)
            if not strategy:
                logger.error(f"❌ Не удалось создать стратегию {best_strategy_name}")
                return None
            
            # === ШАГ 4: ПОЛУЧАЕМ РЫНОЧНЫЕ ДАННЫЕ ===
            enriched_data = await self._enrich_market_data(symbol, market_data)
            
            # === ШАГ 5: АНАЛИЗИРУЕМ ===
            try:
                analysis = await strategy.analyze(enriched_data.get('df'), symbol)
                if not analysis or analysis.action == 'WAIT':
                    logger.debug(f"📊 Стратегия {best_strategy_name} рекомендует ждать")
                    return None
            except Exception as analysis_error:
                logger.error(f"❌ Ошибка анализа стратегии: {analysis_error}")
                return None
            ...
            if combined_confidence < 0.5:
                logger.info(f"📊 Низкая уверенность ({combined_confidence:.1%}), пропускаем")
                return None
            ...
            signal = Signal(
                ...
                price=enriched_data.get('current_price', 0),
    ...
    def _get_pair_config(self, symbol: str) -> Optional[TradingPair]:
        """Получение конфигурации торговой пары - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        db = SessionLocal()
        try:
            ...
            if not pair:
                # Создаем дефолтную конфигурацию с ПРАВИЛЬНЫМИ полями
                pair = TradingPair(
                    symbol=symbol,
                    base_asset=symbol[:-4] if symbol.endswith('USDT') else symbol[:3],
                    quote_asset='USDT' if symbol.endswith('USDT') else symbol[3:],
                    is_active=True,
                    strategy='multi_indicator',      # ✅ ПОЛЕ ЕСТЬ В БД
                    stop_loss_percent=2.0,           # ✅ ПОЛЕ ЕСТЬ В БД
                    take_profit_percent=4.0,         # ✅ ПОЛЕ ЕСТЬ В БД
                    min_position_size=10.0,          # ✅ ПОЛЕ ЕСТЬ В БД
                    max_position_size=1000.0         # ✅ ПОЛЕ ЕСТЬ В БД
                )
                db.add(pair)
                db.commit()
                logger.info(f"✅ Создана дефолтная конфигурация для {symbol}")
    ...
    def _create_strategy_instance(self, strategy_name: str):
        ...
        try:
            ...
            if hasattr(self.strategy_factory, 'create'):
                return self.strategy_factory.create(strategy_name)
    ...
    async def _enrich_market_data(self, symbol: str, market_data: Dict) -> Dict:
        ...
        try:
            # Получаем исторические данные если их нет
            if not market_data.get('df') or market_data['df'].empty:
