...
def create_app():
    ...
    try:
        ...
        register_trading_api_routes(app, bot_manager)
        logger.info("✅ API реальной торговли зарегистрированы")
    except ImportError as e:
        logger.warning(f"⚠️ Не удалось импортировать trading_api: {e}")
    except Exception as e:
        logger.error(f"❌ Ошибка регистрации API реальной торговли: {e}")
    
    # === ВАЛИДАЦИЯ КОНФИГУРАЦИИ ДЛЯ LIVE ТОРГОВЛИ ===
    if not config.PAPER_TRADING and config.LIVE_TRADING:
        try:
            config.validate_trading_config()
            logger.info("✅ Конфигурация для live торговли валидна")
        except ValueError as e:
            logger.error(f"❌ Ошибка конфигурации live торговли: {e}")
            logger.warning("⚠️ Принудительное переключение на paper trading")
