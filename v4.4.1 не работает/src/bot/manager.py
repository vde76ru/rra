...
try:
    ...
    logger.info("✅ BaseStrategy импортирован успешно")
except ImportError as e:
    logger.warning(f"⚠️ Не удалось импортировать BaseStrategy: {e}")
    # Создаем заглушку
    class BaseStrategy:
    ...
    class TradingSignal:
        def __init__(self, action, confidence, price, **kwargs):
            self.action = action
...
class StrategyFactory:
    def create(self, name: str, config: Dict[str, Any] = None, **kwargs) -> BaseStrategy:
        """
        ...
        try:
            # ✅ ИСПРАВЛЕНО: Безопасная передача config
            safe_config = config if isinstance(config, dict) else {}
            
            # Объединяем config и kwargs
            if kwargs:
                safe_config.update(kwargs)
            
            # Создаем стратегию с config
            return strategy_class(config=safe_config)
            
        except TypeError as e:
            # Если конструктор не принимает config, создаем без него
            if "unexpected keyword argument 'config'" in str(e):
                logger.warning(f"⚠️ Стратегия {name} не поддерживает config, создаем без него")
    ...
    async def _generate_signal(self, symbol: str, market_data: Dict[str, Any]) -> Optional[Signal]:
        """
        Исправленная генерация сигналов с правильным созданием стратегий
        """
        try:
            ...
            if not pair_config:
                logger.warning(f"⚠️ Конфигурация для {symbol} не найдена")
                return None
            
            # === ШАГ 2: АВТОМАТИЧЕСКИЙ ВЫБОР СТРАТЕГИИ ===
            strategy_name = 'momentum'  # По умолчанию
            
            if hasattr(self, 'auto_strategy_selector') and self.auto_strategy_selector:
                try:
                    strategy_name, strategy_confidence = await self.auto_strategy_selector.select_best_strategy(symbol)
                    logger.info(f"✅ Выбрана стратегия '{strategy_name}' для {symbol} с уверенностью {strategy_confidence:.1%}")
                except Exception as selector_error:
                    logger.error(f"❌ Ошибка селектора стратегий: {selector_error}")
                    strategy_name = getattr(pair_config, 'strategy', 'momentum')
            
            # === ШАГ 3: СОЗДАЕМ СТРАТЕГИЮ С ПРАВИЛЬНЫМ CONFIG ===
            
            # ✅ ИСПРАВЛЕНО: Создаем config как словарь
            strategy_config = {
                'symbol': symbol,
                'timeframe': getattr(pair_config, 'timeframe', '5m'),
                'min_confidence': 0.6,
                'min_risk_reward': 1.5,
                'position_size': getattr(pair_config, 'position_size', 100),
            }
            
            # Добавляем конфигурацию из глобального config
            if hasattr(self, 'config'):
                strategy_config.update({
                    'max_positions': getattr(self.config, 'MAX_POSITIONS', 1),
                    'stop_loss_percent': getattr(self.config, 'STOP_LOSS_PERCENT', 2.0),
                    'take_profit_percent': getattr(self.config, 'TAKE_PROFIT_PERCENT', 4.0),
                })
            
            # Создаем экземпляр стратегии
            strategy = self._create_strategy_instance(strategy_name, strategy_config)
            
            if not strategy:
                logger.error(f"❌ Не удалось создать стратегию {strategy_name}")
                return None
            
            # === ШАГ 4: ОБОГАЩАЕМ РЫНОЧНЫЕ ДАННЫЕ ===
            enriched_data = await self._enrich_market_data(symbol, market_data)
            df = enriched_data.get('df')
            
            if df is None or df.empty:
                logger.warning(f"⚠️ Нет данных для анализа {symbol}")
                return None
            
            # === ШАГ 5: АНАЛИЗИРУЕМ СТРАТЕГИЕЙ ===
            try:
                trading_signal = await strategy.analyze(df, symbol)
                
                if not trading_signal or trading_signal.action == 'WAIT':
                    logger.debug(f"💤 Сигнал WAIT для {symbol}: {trading_signal.reason if trading_signal else 'None'}")
                    return None
                
                # === ШАГ 6: СОЗДАЕМ СИГНАЛ ДЛЯ БД ===
                signal_data = {
                    'symbol': symbol,
                ...
                if trading_signal.confidence < 0.6:
                    logger.debug(f"💤 Низкая уверенность для {symbol}: {trading_signal.confidence}")
                    return None
                
                if trading_signal.risk_reward_ratio and trading_signal.risk_reward_ratio < 1.5:
                    logger.debug(f"💤 Плохое соотношение риск/прибыль для {symbol}: {trading_signal.risk_reward_ratio}")
                    return None
                
                # Сохраняем сигнал в БД
                signal = await self._save_signal_to_db(signal_data)
                
                logger.info(f"✅ Создан сигнал {trading_signal.action} для {symbol} "
                           f"(уверенность: {trading_signal.confidence:.1%}, "
                           f"стратегия: {strategy_name})")
                
                return signal
                
            except Exception as e:
                logger.error(f"❌ Ошибка анализа стратегией {strategy_name} для {symbol}: {e}")
                logger.exception("Детали ошибки анализа:")
                return None
                
        except Exception as e:
            logger.error(f"❌ Общая ошибка генерации сигнала для {symbol}: {e}")
            logger.exception("Детали общей ошибки:")
            return None
            
            
    async def _save_signal_to_db(self, signal_data: Dict) -> Any:
    ...
    def _get_pair_config(self, symbol: str) -> Any:
        ...
        try:
            ...
            try:
                ...
                # Если нет в БД, создаем дефолтную конфигурацию
                return type('PairConfig', (), {
                    'symbol': symbol,
                    'strategy': 'momentum',
                    'timeframe': '5m',
                    'position_size': 100,
            ...
            logger.error(f"❌ Ошибка получения конфигурации пары {symbol}: {e}")
            # Возвращаем минимальную конфигурацию
            return type('PairConfig', (), {
                'symbol': symbol,
                'strategy': 'momentum',
                'timeframe': '5m',
    ...
    def _create_strategy_instance(self, strategy_name: str, config: Dict = None):
        """
        Создание экземпляра стратегии с безопасной обработкой config
        
        Args:
            strategy_name: Название стратегии
            config: Конфигурация стратегии (должен быть словарь!)
            
        Returns:
            BaseStrategy или None
        """
        try:
            ...
            if config is None:
                ...
                logger.error(f"❌ ОШИБКА: Получена строка вместо dict для config стратегии {strategy_name}!")
                logger.error(f"❌ Переданное значение: '{config}' (тип: {type(config)})")
                logger.error("❌ Исправьте код, который передает config как строку!")
                safe_config = {}
            else:
                logger.error(f"❌ Неожиданный тип config: {type(config)} = {config}")
                safe_config = {}
            
            logger.debug(f"🏭 Создание стратегии {strategy_name} с config: {list(safe_config.keys())}")
            
            # Пробуем создать через фабрику
            if hasattr(self.strategy_factory, 'create'):
                return self.strategy_factory.create(strategy_name, config=safe_config)
            elif hasattr(self.strategy_factory, strategy_name):
                # Прямое обращение к атрибуту (старый способ)
                strategy_class = getattr(self.strategy_factory, strategy_name)
                try:
                    return strategy_class(config=safe_config)
                except TypeError:
                    # Если не поддерживает config
                    return strategy_class()
            else:
                logger.error(f"❌ Стратегия {strategy_name} не найдена в фабрике")
                return None
    ...
    async def _enrich_market_data(self, symbol: str, market_data: Dict) -> Dict:
        """Обогащение рыночных данных"""
        try:
            # ✅ ИСПРАВЛЕНО: Правильная проверка DataFrame
            df = market_data.get('df')
            if df is None or df.empty:
                market_data['df'] = await self._fetch_market_data(symbol)
            
            # Добавляем текущую цену
            if not market_data.get('current_price'):
                market_data['current_price'] = await self._get_current_price(symbol)
