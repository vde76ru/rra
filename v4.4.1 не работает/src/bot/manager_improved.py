...
class ImprovedBotManager:
    ...
    async def start_trading(self):
        ...
        for comp_name in required_components:
            # Проверяем наличие компонента и его статус
            if comp_name not in component_manager.components:
                logger.error(f"❌ Компонент {comp_name} не зарегистрирован")
                return False
            
            component_info = component_manager.components[comp_name]
            if component_info.status != ComponentStatus.READY:
                logger.error(f"❌ Компонент {comp_name} не готов. Статус: {component_info.status.value}")
                return False
