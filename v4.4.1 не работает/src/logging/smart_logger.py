from src.core.models import TradingLog
"""
...
class SmartLogger:
    ...
    def error(self, message: str, category: str = 'general', **context):
        """Логирование ошибки"""
        # ИСПРАВЛЕНИЕ: проверяем exc_info перед использованием
        exc_info = context.pop('exc_info', None)
        if exc_info is True:
            import sys
            exc_info = sys.exc_info()
        
        self.log('ERROR', message, category, exc_info=exc_info, **context)
