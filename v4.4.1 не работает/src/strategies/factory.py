...
Путь: src/strategies/factory.py
"""
...
class StrategyFactory:
    ...
    def __init__(self):
        ...
        logger.info(f"✅ StrategyFactory инициализирована с {len(self._strategies)} стратегиями")
    ...
    def create(self, name: str, config: Dict[str, Any] = None, **kwargs) -> BaseStrategy:
        ...
        try:
            ...
            if config is None:
                ...
                # ❌ ОШИБКА: Если передана строка вместо словаря
                logger.error(f"❌ КРИТИЧЕСКАЯ ОШИБКА: Получена строка вместо dict в create()!")
                logger.error(f"❌ Стратегия: {name}, config: '{config}' (тип: {type(config)})")
            ...
            logger.debug(f"🏭 Создание стратегии {name} с config: {list(safe_config.keys())}")
            
            # Создаем стратегию с config
            try:
                strategy = strategy_class(config=safe_config)
                logger.info(f"✅ Создана стратегия: {name}")
                return strategy
            except TypeError as te:
                # Если конструктор не принимает config, пробуем создать без него
                if "unexpected keyword argument 'config'" in str(te):
                    ...
                    # Пытаемся установить config после создания
                    if hasattr(strategy, 'config') and safe_config:
                        strategy.config.update(safe_config)
                    return strategy
                else:
                    raise te
            ...
            logger.error(f"❌ Переданные параметры: config={config}, kwargs={kwargs}")
            raise
    ...
    def get_strategy_info(self, name: str) -> Optional[Dict]:
        ...
        elif hasattr(strategy_class, 'TYPE'):
            info['type'] = strategy_class.TYPE
        else:
            info['type'] = 'general'
...
__version__ = "1.1.0"  # Увеличили версию после исправлений
