...
import json
from src.core.bot_manager import bot_manager  # Основной менеджер
...
from src.core.models import Trade, User, BotState, TradingPair, Signal, TradeStatus
from src.core.process_bot_manager import process_bot_manager
        ...
        async function updatePairs() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    showNotification('Failed to update pairs', 'error');
                }
            } catch (error) {
                showNotification('Connection error', 'error');
            } finally {
...
async def get_dashboard_data_internal():
    ...
    try:
        # Получаем статус бота через процессный менеджер
        bot_status = await process_bot_manager.get_bot_status()
        ...
        return {
            ...
            'max_positions': bot_manager.max_positions,
            'total_trades': total_trades,
            'win_rate': win_rate,
            'bot_running': bot_status['is_running'],  # Из процессного менеджера
            'active_pairs': bot_status['active_pairs'],  # Из процессного менеджера
...
async def control_bot(action: str, current_user: User = Depends(auth_service.get_current_user)):
    if action == "start":
        success = await process_bot_manager.start_bot()
        if success:
...
async def update_trading_pairs(
    ...
    success = await process_bot_manager.update_pairs(pairs.get('pairs', []))
    return {"status": "updated", "pairs": pairs.get('pairs', [])}
...
async def close_position(
    ...
    success = await process_bot_manager.close_position(symbol)
    if success:
