...
Статус: КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ управления состоянием
...
4. Устранение десинхронизации состояний
"""
...
class BotController:
    ...
    async def _start_real_bot(self, pairs: List[str], strategy: str) -> bool:
        ...
        try:
            ...
            if hasattr(self.bot_manager, 'start'):
                success, message = await self.bot_manager.start()
    ...
    async def _stop_real_bot(self) -> bool:
        """Остановка реального BotManager"""
        if not self.bot_manager:
            logger.warning("BotManager не доступен, используем симуляцию")
        ...
        try:
            if hasattr(self.bot_manager, 'stop'):
                success, message = await self.bot_manager.stop()
    ...
    async def stop_bot_action():
        ...
        try:
            ...
            if result['success']:
                ...
                logger.warning(f"⚠️ Остановка с ошибками: {result['message']}")
    ...
    async def restart_bot_action():
        ...
        try:
            ...
            if result['success']:
                ...
                logger.error(f"❌ Перезапуск неудачен: {result['message']}")
