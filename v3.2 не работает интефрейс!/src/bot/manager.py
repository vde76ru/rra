...
# ИСПРАВЛЕНИЕ: Убираем дублированные импорты
...
# ИСПРАВЛЕНИЕ: Используем try/except для опциональных модулей
try:
...
class BotManager:
    ...
    def _init_components(self):
        ...
        try:
            ...
            # Импортируем улучшенный менеджер
            from .manager_improved import ImprovedBotManager
    ...
    def _init_components_basic(self):
        """Базовая инициализация компонентов (как backup)"""
        ...
        try:
            ...
            if self.exchange:
                ...
                self.analyzer = None
                logger.warning("⚠️ Market Analyzer пропущен (нет Exchange)")
        except ImportError:
            logger.info("📦 Market Analyzer не найден (не критично)")
            self.analyzer = None
        ...
        try:
            ...
            if self.exchange:
                ...
                self.trader = None
                logger.warning("⚠️ Trader пропущен (нет Exchange)")
        except ImportError:
            logger.info("📦 Trader не найден (критично)")
            self.trader = None
    ...
    async def _update_bot_state(self):
        """
        Асинхронное обновление состояния бота в базе данных
        
        Данный метод служит асинхронной оберткой для существующего
        синхронного метода _update_bot_state_db() и обеспечивает:
        
        1. Автоматическое определение текущего статуса бота
        2. Корректное обновление записи в таблице bot_state
        3. Обработку исключений с детальным логированием
        4. Совместимость с асинхронной архитектурой системы
        
        Алгоритм работы:
        - Анализ текущего статуса бота (self.status)
        - Определение флага is_running для БД
        - Вызов синхронного метода обновления
        ...
        Raises:
            None: Все исключения обрабатываются внутри метода
            
        Usage:
            await self._update_bot_state()
            
        Dependencies:
            - self._update_bot_state_db() (должен существовать)
            - self.status (BotStatus enum)
    ...
    def _determine_bot_running_status(self) -> bool:
        ...
        Анализирует текущее состояние бота и возвращает булевый флаг
        для корректного обновления поля is_running в таблице bot_state.
        
        Логика определения:
        - RUNNING, STARTING -> True (бот активен)
        - STOPPED, STOPPING, ERROR -> False (бот неактивен)
        - Отсутствие атрибута status -> False (безопасное поведение)
        
        Returns:
            bool: True если бот должен считаться запущенным в БД
        """
        try:
    ...
    async def _sync_additional_statistics(self):
        ...
        Опциональный метод для синхронизации расширенной статистики
        после основного обновления состояния. Включает:
        
        - Обновление счетчиков сделок
        - Синхронизацию времени работы
        - Обновление метрик производительности
        
        Метод разработан как non-blocking и не влияет на основную логику
